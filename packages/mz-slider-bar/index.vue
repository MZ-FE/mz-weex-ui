...
export default {
  ...
  watch: {
    value(newVal) {
      if (!this.range) {
        ...
        this.diffX1 = this._getDiffX(newVal[0] || this.defaultValue[0]);
  ...
  computed: {
     ...
     */
    startValidColor() {
      return Array.isArray(this.validColor) && this.validColor.length >= 2
        ? this.validColor[0]
        : "";
    },
    /**
    ...
    blockStyle1() {
      let left = this.diffX1,
        style = {
          ...
          height: `${this.pointHeight}px`,
          transform: `translateX(${left < 0 ? 0 : left}px)`,
  ...
  methods: {
     ...
     */
    weexEndBar(e) {
      if (!this.isCanClick || this.disabled) {
        //禁止点击
        return;
      }
        ...
        const gesTokenObj = Binding.bind(
          ...
          (e) => {
            if (
              ...
              const range = self.getRange();
              self.barWidth = self._restrictValue(
                range.rangeX1,
                self.barWidth - e.deltaX
              );
              self.diffX1 = self._restrictValue(
                range.rangeX1,
                self.diffX1 + e.deltaX
      ...
      const gesTokenObj = Binding.bind(
        ...
        (e) => {
          if (e.state === "end" || e.state === "cancel" || e.state === "exit") {
            const range = self.getRange();
            self.barWidth = self._restrictValue(
              [0, self.length - self.diffX1],
              self.barWidth + e.deltaX
            );
            self.diffX2 = self._restrictValue(
              range.rangeX2,
              self.diffX2 + e.deltaX
            );
