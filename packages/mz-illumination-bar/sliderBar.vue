<template>
  <div ref="bar-container" class="slider-bar-container" :style="containerStyle">
    ...
    <div class="click-bar" :style="clickBarStyle" v-if="false">
      <div
        class="item-block"
        :style="itemBlockStyle"
        v-for="(item, index) in clickItemList"
        :key="index"
        v-on:click="onSliderClick(index)"
      ></div>
    </div>
...
export default {
  ...
  computed: {
    ...
    itemBlockStyle () {
      let calc_width = parseInt((this.length + 56) / (this.max - this.min + 1))
      let width = calc_width >= 56 ? 56 : calc_width
      return {
        width: `${width}px`,
        height: '56px'
      }
  ...
  methods: {
    ...
    weexHandler1 (e) {
      ...
      switch (e.state) {
        ...
        case 'end':
          if (!this.disabled) {
            setTimeout(function () {
              dom.getComponentRect(self.block1, option => {
                const { left } = option.size
                const value = self._getValue(left - self.leftDiffX)
                if (!self.range) {
                  self.$emit('slideEnd', value)
                } else {
                  self.selectRange[0] = value
                  self.$emit('slideEnd', self.selectRange)
                }
              })
            }, 150)
          }
    ...
    weexHandler2 (e) {
      ...
      switch (e.state) {
        ...
        case 'move':
          ...
          break
        case 'end':
          if (!this.disabled) this.$emit('slideEnd')
          break
        default:
          break
      }
    ...
    bindBlock1 () {
      ...
      let step = self.length / (self.max - self.min)
      let stepHalf = step / 2
        ...
        const props = [
          {
            ...
            expression: self.stepMode
              ? `min(${blockMax1}, max( floor((${self.diffX1}+x)/${stepHalf})%2 == 0 ? (floor((${self.diffX1}+x)/${step})*${step}) : (ceil((${self.diffX1}+x)/${step})*${step}) , 0))`
              : `min(${blockMax1}, max(x + ${self.diffX1}, 0))`
          },
          {
            ...
            expression: self.stepMode
              ? `min(0, max( floor((${startLeft}+x)/${stepHalf})%2 == 0 ? (floor((${startLeft}+x)/${step})*${step}) : (ceil((${startLeft}+x)/${step})*${step}) , -${blockMax1}))`
              : `min(0, max(x + ${startLeft}, -${blockMax1}))`
          }
        ]
        ...
        const gesTokenObj = Binding.bind(
          ...
          e => {
            if (e.state === 'end' || e.state === 'cancel' || e.state === 'exit') {
              ...
              if (self.stepMode) {
                self.diffX1 = self._restrictValue(
                  range.rangeX1,
                  Math.floor((self.diffX1 + e.deltaX) / stepHalf) % 2 === 0
                    ? Math.floor((self.diffX1 + e.deltaX) / step) * step
                    : Math.ceil((self.diffX1 + e.deltaX) / step) * step
                )
              } else {
                // 限制diffX1范围
